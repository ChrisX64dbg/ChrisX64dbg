@echo off 
mode 200 
title [ Chris Tool ] Autmated Apps Installer! 
color 1f 
reg ADD "HKEY_CURRENT_USER\SOFTWARE\DM Windows Optimizer\Updater" /v "AutomatedApps" /t REG_SZ /d "3.0.0" /f >nul 2>&1 
...
:AppsUtilityMenu 
cls 
echo ... Welcome %username% to ChrisTool Autmated Apps Installer^/Uninstaller ... 
echo. 
echo. 
echo 1. Install Apps 
...
echo 999. Exit 
echo. 
Echo Saved Config File will be on your Desktop named ChrisToolApps.config move it to another safe place. 
echo To Load Config File, simply copy your previously saved ChrisToolApps.config to Desktop and relunch the app. 
echo. 
echo. 
...
if %answer%==3 goto ChrisToolAutoAppUpdater 
if %answer%==4 goto SaveChrisToolConfig 
if %answer%==5 goto LoadChrisToolConfig 
if %answer%==999 goto Exit 
goto AppsUtilityMenu 
:Exit 
cls 
echo Thank you  %username% for using ChrisTool Automated Apps Installer^/Uninstaller^! 
echo. 
timeout /t 3 
Exit 
...
:AppsInstaller 
findstr /l /x /c:"%Package%" "%userprofile%\ChrisToolAppsInstaller.config" >nul 2>&1 
IF NOT ERRORLEVEL 1 goto :eof 
echo installing %Package% Please Wait... 
Choco install %Package% -y -r --allowemptychecksum --allowemptychecksumsecure --ignore-checksums
del /f /s /q "%userprofile%\ChrisToolAppsInstaller.config" >nul 2>&1 
choco list -l -r --id-only > "%userprofile%\ChrisToolAppsInstaller.config" 
echo Done. 
goto :eof 
:AppsUninstaller 
findstr /l /x /c:"%Package%" "%userprofile%\ChrisToolAppsInstaller.config" >nul 2>&1 
...
call %CallAppsInstallorUninstall% 
goto :eof 
:ChrisToolAutoAppInstallerFromConfig 
for /F "tokens=* USEBACKQ" %%i in (%userprofile%\ChrisToolAppsInstaller.config) do ( 
cls 
echo Installing %%i Please Wait... 
Choco install %%i -y -r --allowemptychecksum --allowemptychecksumsecure --ignore-checksums
echo Done, Installed Successfully. 
) 
goto ASKCheckingForUpdate 
:ChrisToolAutoAppUpdater 
cls 
...
:SaveChrisToolConfig 
IF EXIST "%userprofile%\ChrisToolAppsInstaller.config" ( 
cls 
echo Grapping Current Config File... 
TIMEOUT 1 >nul 
cls 
echo Copying Current Config File to Desktop.. 
choco list -l -r --id-only > "%userprofile%\Desktop\ChrisToolApps.config" 
...
:LoadChrisToolConfig 
IF EXIST "%userprofile%\Desktop\ChrisToolApps.config" ( 
cls 
echo Checking for New Config File... 
TIMEOUT 1 >nul 
cls 
echo Loading New Config File.. 
copy "%userprofile%\Desktop\ChrisToolApps.config" "%userprofile%\ChrisToolAppsInstaller.config" /-y/v 
echo New Config Loaded Successfully. Going to Install NOW^! 
TIMEOUT 1 >nul 
goto ChrisToolAutoAppInstallerFromConfig 
) ELSE ( 
cls 
echo Checking for New Config File... 
TIMEOUT 1 >nul 
...
echo if you want to load Config from Desktop just add one and rename it to "ChrisToolApps.config" .
...
:break 
if errorlevel 2 (goto ChrisToolAutoAppUpdater) else goto EOFAPPNoUpgrade 
:EOFAPPInstaller 
cls 
